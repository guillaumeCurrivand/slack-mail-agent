...
For a Module-owned list that edits its own menu message, return `bindButtons: true` on the `MenuPage` and declare the local action names in `menuActions`. The shared runtime binds each button value to the recorded menu and carries the signed Agent-message timestamp into the durable action job. In the handler, call `Navigation.boundTarget` with the authenticated actor, value and timestamp before updating; use `Navigation.show` with its returned target to edit that list. Recheck current Module state and access before each mutation. Persist an event-ID checkpoint atomically with each data change so replaying a delivered action cannot repeat it after a later, opposite action. The Module owns the list and data changes; core owns target authorization and delivery recovery.

For an explicit work-start button among ordinary menu buttons, mark that button `bound: true`, declare its local action in `menuActions`, and validate its bound menu identity again in the handler before entering the existing workflow. Declare a stable internal `key`, user-facing `label`, matching exact typed commands and action in `workOperations` so ingress admits both routes through the same owner-scoped operation slot. If a Module can infer the operation later from natural language, set `snapshotUnknownText` and atomically claim the operation after resolving intent; Mail Sorter demonstrates this. Use this only for deliberate on-demand work; do not register read-only navigation or approval actions. A duplicate request becomes a separate private status Card tied to the original job ID. The original Module still owns prerequisites, provider checkpoints, results and approval safeguards.
...
- `cleanup(pool)`: remove expired module state. Use `withOwner` with the Module ID as its scope before changing active per-user records. Cleanup runs at startup and hourly; shutdown waits for an active cleanup to finish.

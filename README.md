...
A private Slack assistant for a 10-person Google Workspace team. Users connect their own Gmail account, approve personal rules, request a preview of their latest 100 inbox messages, and confirm before any messages are labeled, archived, or moved to Trash.
...
- Hourly retention cleanup of conversations and run records older than 30 days; expired login states and rule proposals are also removed.
...
Each teammate sends `connect` in a DM, follows their short-lived link, grants access using their own Workspace account, and confirms the resulting email address back in Slack. Authorization is not activated until this last confirmation. An account cannot be activated for two Slack users in the configured workspace.

`disconnect` removes the application's active credentials and cancels pending previews after confirmation. It leaves rules in place. Users can additionally revoke the OAuth grant in their Google account. Historical encrypted credentials may remain in database backups until their retention period expires.
...
The app counts the immutable input through the Responses token-count endpoint, reserves exact input plus bounded output in integer microdollars, then generates with `store:false`, standard service tier, disabled truncation, and no automatic retries. It conservatively charges all input at the uncached rate. Both conversation and classification calls use the same team ledger.
...
Send these in a private conversation with the bot:

- `connect`: connect Gmail.
- `starters`: review Urgent and Newsletter templates; describe project names and exact sender addresses separately.
- `rules`: inspect approved rules and their IDs.
- `sort`: scan the latest 100 inbox messages and receive a preview.
- `report`: inspect the latest run, open detailed pages, or undo a completed run.
- `details <run-id> <page>`: open a retained run's details (page numbers start at zero).
- `budget`: see shared recorded and reserved AI usage without paying for a model call.
- `disconnect`: propose disconnecting Gmail.

Natural-language examples: “Label messages from alex@example.com as Projects/Alpha and keep them in my inbox.” “Change my newsletter rule to exclude product announcements.” “For message `<message-id>` in run `<run-id>`, remove the Urgent label and keep it in my inbox.” Corrections have their own confirmation preview; a future-rule change requires separate approval.
